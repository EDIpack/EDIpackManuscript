...
The main one is a static Fortran library calledc{\tt  libedipack.a} which, alongside the compiled
modules, wraps the \NAME software in both its single- and multi-impurity variants.
A second dynamic library, {\tt libedipack\_cbinding.so}, together with the associated header
file{\tt edipack2\_cbinding.h}, enables interoperability with other programming languages.  
...
The default target builds and install both the main library and the C-bindings.
Separate build targets for either are available. A recap message is printed at the end of the
CMake configuration step. 

\subsubsection{Anaconda}
As an alternative we provide for both Linux and Mac Os systems
