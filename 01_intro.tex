...
Most notably, quantum impurity problems lie at the core of many quantum embedding methods, like dynamical mean-field theory (DMFT)~\cite{Georges1996RMP}, density-matrix embedding theory (DMET)~\cite{Knizia2012PRL} and the Gutzwiller approximation (GA)~\cite{Lanata2015PRX}, where the complex behavior of the lattice systems is reduced to an effective self-consistent quantum impurity problem. The present code has been developed with a particular focus for DMFT but its usage extends also to the aforementioned methods.
}
...
\fixme{(Samuele) Numerous exact diagonalization (ED) open-source software packages have been developed by the scientific community addressing the quantum many-body problem, with a strong emphasis on both efficiency and flexibility. Notably, these include codes designed for general systems of interacting fermions, such as EDLib~\cite{Iskakov2017} and the more recent XDiag~\cite{Wietek2025}; for systems involving both fermions and bosons, such as Pomerol~\cite{Antipov2015}; and for fermions coupled with spins, such as H$\Phi$~\cite{Kawamura2017CPC,Ido2024CPC}.
