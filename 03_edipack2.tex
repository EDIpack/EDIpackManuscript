...
\fixme{There's no actual reason to not provide the ``elemental" norm for
the normal and hybrid bath parametrizations, as it is relevant or not
depending on the off-diagonal structure of $\Theta(z)$, which depends
only on \texttt{ed\_mode}, not \texttt{bath\_type}. What do we do? I would
vote for describing what the different norms do and vaguely mention that
they do not work for all possible \texttt{bath\_type} values, without 
entering any detail on the reason why. Or even not mention it, and slowly
start to update the code to make it work with all possible combinations...}
%GBtodo placeholder
