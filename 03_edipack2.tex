...
\end{equation}
and represents the quadratic part, with $d_{\a\sigma}$ ($d^+_{\a\sigma}$) are the annihilation (creation)
...
\begin{equation}
\hat{H}^\mathrm{int}_\mathrm{imp} = \frac{1}{2} \sum_{ijkl} c^{\dagger}_{i}c^{\dagger}_{j}U_{ijkl}c_{l}c_{k}.\label{HintUmat}
\end{equation}
...
\label{Hint}
\end{equation}
where we introduced the occupation number operator   $n_{\alpha\sigma}=d^{+}_{\alpha\sigma}d_{\alpha\sigma}$.
...
$V^p_{\a\b\s\sigma'}$ desribes the coupling with the impurity.   
...
\begin{equation}\label{Hph}
  \begin{split}
    \hat{H}_\mathrm{ph}&=\sum_m \omega_{0m} b_m^+b_m   \\
    % 
    \hat{H}_\mathrm{e-ph} &= \sum_m g_m[\hat{O}_m - A_m](b_m+b_m^+) \\
\end{split}
\end{equation}
where $m=1,\dots,M$ indexes the number of local phonon modes, and $b_m$
($b_m^+$) are the destruction (creation) operators for a phonon with
frequency $\omega_{0m}$. Each phonon mode is coupled to a bilinear fermionic operator $\hat{O}_m$ on the impurity, while $A_m$ is a constant acting as a displacement field.  %Indeed, if the expectation value of the bosonic displacement operator $\langle b_q^\dagger + b_q\rangle$ is non-zero, a change of variables from the bosons $b^{(\dagger)}_q$ leads to new effective bosons $\tilde{b}^{(\dagger)}_q = b^{(\dagger)}_q- A_q$ for which the displacement operator has a smaller expectation value (ideally zero if we can impose $A_m = \langle \hat{O}_m\rangle$). 
For single-orbital impurities, the relevant case is $O_1 = n_d = \sum_{\sigma} d^{\dagger}_{\sigma}d_{\sigma}$, which defines the Holstein model\cite{Holstein1959APNY}. For two-orbital models we included also the two Jahn-Teller modes\cite{Capone2010ACMP}, for which $O_2 = n_1 - n_2$\cite{Scazzola2023PRB} and $O_3 = \sum_{\sigma} (d^{\dagger}_{1\sigma}d_{2\sigma}+ h.c.)$. For each phonon mode, one has to introduce a cut-off in the phonon number which has to be chosen according to the value electron-phonon coupling\cite{Capone2003PRL,Capone2006PRB} and its interplay with the electron-electron interaction\cite{Capone2004PRL_2,Sangiovanni2005PRB,Paci2005PRL,Sangiovanni2006PRL,Sangiovanni2006PRB_ph,Paci2006PRB}. The larger the number of excited phonons in the groundstate, the larger the cut-off. For practical calculations around 20 phonons are sufficient to achieve converged results except for the deep polaronic regime\cite{Capone2003PRL}.
Choosing properly the shift $A_m$ one can reduce the number of excited phonons in the groundstate and consequently the required cut-off. 
Although feasible, dealing with more than one phonon mode becomes quickly computationally very demanding, thus in the rest of the this work we will consider only one mode.

The phonon frequency is passed to the \NAME program via the variable { \tt W0\_PH } to be read from the input file. The displacement field acting on the phonon is passed through the variable {\tt A\_PH} from the input file. For every phonon mode, the $g_m$ coupling can be written as a matrix $g_{\alpha \beta}$ which can be passed in two different ways. If the matrix is diagonal ($g_{\alpha \beta} = g_\alpha \delta_{\alpha \beta}$), from the input file one can simply write in order the values $g_\alpha$ in the variable {\tt G\_PH }. For any general shape one can pass to the input file a {\tt GPHfile } variable containing the name of the file storing the $g_{\alpha \beta}$ array in Fortran style.
...
The total size of the system is determined by three contributions: i) the number of impurity orbitals $N_\a$, ii) the total count of electronic bath levels $N_\mathrm{b}$, and iii) by maximum  number of allowed phonons $N_\mathrm{ph}$, introducing a cut-off to the unbounded dimensions of the local phonon Hilbert space.
...
The total number of electronic levels in the quantum impurity systems is indicated with $N_\mathrm{s}$.
...
The electornic destruction and creation operators, $c_{a\sigma}$ and $c^+_{a\sigma}$ 
...
The bosonic operators $b$ and $b^\dagger$ act on the phonon part of $\ket{p}$ of the Fock state as:
\begin{align*}
...
As the symmetries applies to the electronic terms, here we discuss the organization of the electronic Fock states only. The presence of phonon is accounted for exploiting the tensor structure of the global Fock space.  


In \NAME, three different symmetry configurations are available, 
controlled by the input variable {\tt ed\_mode} = {\bf normal}, 
{\bf superc}, or {\bf nonsu2}. A summary of the sector properties is provided in
Table~(\ref{TabSector}). 
...
can be extended to act independently to each orbital and spin
...
$\MM:\SS_{\vec{Q}}\rightarrow \FF_e$ which relates the electronic states $\ket{i}$
...
of the quantum numbers $\vec{Q}$, i.e. the value of {\tt ed\_mode}. The following  code snippet summarize the various available implementations. The core idea is to loop of necessary Fock states through the integers $I_\sigma=1,\dots,2^{N_s}$ and enforce the quantum number constraint:
...
\NAME offers two distinct methods to setup the interaction terms $\hat{H}^\mathrm{int}$ in \equ{Himp}, which are controlled by the input variables {\tt
...
Alternatively, setting {\tt ED\_USE\_KANAMORI=F} and {\tt ED\_READ\_UMATRIX=T}, a list of two-body operators can be supplied by the user in a properly
...
...
\end{lstlisting}
where {\tt NORB} is the numer of orbitals $N_\a$.\\
Each line defines a two-body second-quantized operator of the form 
$$
\hat{H}^{\mathrm{int}}=\frac{1}{2}c^{\dagger}_{i}c^{\dagger}_{j}U_{ijkl}c_{l}c_{k}.
$$
...
disjoint information about the corresponding electronic Fock state $\ket{I_e}$. 
...
These relations ensure a better scalign and can be easily inverted to obtain $I_{\sigma=\up,\dw}$ from
...
An overview of the implementation, complementing the symmetry sector construction, is reported in the following listing:
\setbox0=\hbox{%
...
leveraging the Fortran intrinsic function {\tt ibits(I,pos,len)} which returns the integer representation of the bit subset of length {\tt len} starting from {\tt pos} of the  integer {\tt I}.     
...
degeneracy) is stored, for finite temperatures the list  also include all the necessary excited states.
...
type sparse_espace
   ...
   real(8)                     :: emax               !Max energy  fixed by ${e^{-\beta {\tt emax}}<{\tt cutoff}}$
...
groundstate energy,  $\beta=1/T$ is the inverse temperature ($k_B=1$) and \texttt{cutoff} is an input parameter fixing an {\it a-priori} energy threshold. 
...
functions (DCF), e.g. $C(t) = \ibra \TT[ A(t) A^+ ]\iket$. Using Krylov methods,  
...
\begin{lstlisting}[style=fstyle,numbers=none]   
!The contributions to the GF K\"all\'en-Lehmann sum are stored as
...
!and a given operator in the K\"all\'en-Lehmann representation.  
type GFchannel
...
a user-defined matrix basis of dimension $N_\mathrm{sys}$ 
...
high-performance diagonalization algorithms. In \onlinecite{Amaricci2022} we discussed in greater details the distributed-memory massively parallel strategies currently employed in \NAME to optimize this step and present various benchmarks of the parallel scaling. 
Here, we provide a brief outline of the main ideas following the notation introduced
in \onlinecite{Amaricci2022}, and focusing on the impact of the choice of {\tt ed\_mode}. For sake of clarity, here we focus on the purely electronic case, i.e. neglecting the electron-phonon part. The product structure of the electron and phonon Fock spaces ensure that the following approach applies with few algorithmic modifications in presence of electron-phonon coupling.  
