...
one of four possible values: {\bf normal}, {\bf hybrid}, {\bf
  ...
  \fixme{This figure looks quite annoying to generate, especially to cover
  all possible \texttt{ed\_mode} values. I'd pass...}
...
impurity orbitals, resulting in a total of % Ocio!
...
%   \hline
%   {\bf replica} $[N_\mathrm{b}\!=\!N_\mathrm{bath}N_\mathrm{sym}]$ & \multicolumn{3}{*{\hat{h} } \cr
%   \hline
%   {\bf general} $[N_\mathrm{b}\!=\!N_\mathrm{bath}N_\mathrm{sym}]$ & \cline{2-4} & b&  c  \cr
%   \hline
% \end{tabularx}
% \end{center}
% \caption{Ciao}
...
% replacement of the pure state average %%> What is a pure state average?
...
%%% ATTENZIONE: this is true only for the normal case!
%
...
%
%%% I WOULD SAY THAT WE DON'T NEED TO REPEAT OURSELVES< THE SECTOR
%%% SECTION IS THERE FOR A REASON. I try to edit below:
...
\fixme{After careful inspection, I (GB) realize that the elemental norm 
was never really ported from CDFMT-ED to EDIpack (there's no different
matrix weight than all ones, which is the standard chi2, of course). 
Hence the \texttt{cg\_norm} options is really not doing anything significant,
as with "elemental" it gives the usual $\chi^2$ (or $\chi^q$, if you change the
\texttt{cg\_pow}...) and for "Frobenius" it gives again the usual $\chi^2$,
unless you put $q\neq2$, which gives an ill-defined matrix norm...
(it mixes an effective $q=2$ coming from the matrix multiplication with 
$q\neq2$ when you do the explicit $q$-root). I think we
should just get rid of \texttt{cg\_norm} and do the traditional $\chi^q$, as 
indeed we are already doing by default. Implemented like this, it is just random
no-consequence code.}
%GBtodo placeholder
