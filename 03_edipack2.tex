...
adopt a generic formulation of the local multi-orbital Hubbard-Kanamori interaction~\cite{Georges2013AROCMP}:
\begin{equation}
  \begin{split}
...
$U'$ the inter-orbital one and $J$ the Hund's coupling~\cite{Georges2013AROCMP}.  
...
$J_X=J_P=J$~\cite{Georges2013AROCMP}. Different choices, preserving part
of the combined symmetry group, can be made for other numbers of orbitals~\cite{Georges2013AROCMP}.
...
of the quantum numbers $\vec{Q}$, i.e. the value of {\tt ed\_mode}. The following code snippets summarize the various available implementations. The core idea is to loop over the necessary Fock states through the integers $I_\sigma=1,\dots,2^{N_s}$ and to enforce the quantum number constraint:
...
% \fixme{General question: Could it be more instructive to use the left bit shift notation in some expression, e.g. $A \ll N$ instead of $A \cdot 2^N$?}
...
It leverages the Fortran intrinsic function {\tt ibits(I,pos,len)} which returns the integer representation of the bit subset of length {\tt len} starting from {\tt pos} of the  integer {\tt I}.     
...
% \fixme{ (Samuele) Here and before h with or without hat? I would say without since it is how we wrote it in 3.1.1}
% \fixme{The superc h should have tau\_z instead of tau\_0 in nambu repr}
...
high-performance diagonalization algorithms. In \onlinecite{Amaricci2022} we discussed in greater details the distributed-memory massively parallel strategies currently employed in \NAME to optimize this step and presented various benchmarks of the parallel scaling. 
...
\equ{eq:thermal_average}.What is the meaning of $\langle\rangle$ here? \equ{eq:thermal_average} contains a weighted sum over the states of the full system, while $\rho^\mathrm{imp}$ is already reduced over the bath degrees of freedom.
...
indices.
% \fixme{(igor and lorenzo) to be honest we took a while to understand this equation. There are a lot of indices... could it be rewritten more compactly e.g. by not explicitly writing |p><q|? Or perhaps by only showing one spin species, since the Fock space factorizes? Or alternatively, maybe color-code the parts? I get that
% sum\_p sum\_q <b|p><q|b> is a trace... but it's not immediate}
