...
\subsection{Interacting Kane-Mele model (Fortran API, EDIpack2ineq layer)}

To complete the overview of \NAME capabilities, we consider another
model for interacting quantum spin-Hall insulators, defined on the honeycomb lattice, with two inequivalent sites in the unit-cell.

Neglecting the Rashba spin-orbit interaction and discarding any ionic character of the unit-cell, the noninteracting Kane-Mele Hamiltonian \cite{Kane2005PRLa,Kane2005PRL} can be written in momentum space as
\begin{align*}
...
the amplitude of a complex next-nearest-neighbor hopping amplitude,
with a spin-dependent $\pm\tfrac{\pi}{2}$ chiral phase arising from spin-orbit coupling and well-known to stabilize a topological gap at the Fermi level \cite{Kane2005PRLa,Kane2005PRL}.
...
For $\lambda_\mathrm{so}=0$ the model describes a Dirac semimetal, up to relatively high Hubbard repulsion, and magnetizes to an isotropic Néel state above above a critical $U/t$ ratio
\footnote{In contrast with the expectations for a generic half-filled bipartite lattice, for which the groundstate is antiferromagnetic for any nonzero interaction, this property descends from the peculiar properties of Dirac cones: the vanishing density of states
...
that this amount to having $N_\mathrm{sym}=2$, with
$\lambda_{1,p} \equiv \epsilon_p$ describing average
bath levels and $\lambda_{2,p} \equiv m_p$ allowing 
a magnetic split, respectively in the $S_z$ and $S_x$ spin bases.
...
degree of accuracy of our total energy calculation, we benchmark against
...
Ref.\,\cite{Racz2020}. The total energy comparison is reported in 
\figu{fig:KMenergy}(b), for $\lambda_\mathrm{so}=0$.
...
\hspace{1cm} (a) \hspace{7.25cm} (b)\\
    ...
    \caption{In panel (a) the total energy difference between the in-plane 
    and off-plane AFM solutions found in DMFT-ED at $T=0$. Negative values
