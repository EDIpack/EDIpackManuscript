...
discussion of relevant code snippets. As some tasks are inherently agnostic on the impurity
solver (and the impurity solver is agnostic about them), the user is expected to implement
them, or interface to existing utilities. For instance, all the Fortran subroutines with 
naming convention \texttt{dmft\_do\_something()} refer to implementations that can be found
in the {DMFTtools} package\footnote{\url{https://github.com/aamaricci/DMFTtools}},
the example based on w2dynamics relies on the internal facilities provided by the \texttt{DMFT.py} script \cite{Wallerberger2019CPC} and the TRIQS library provides an 
extensive toolbox for the manipulation of Green's functions and related quantities
\cite{Parcollet2015CPC}.
