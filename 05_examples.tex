...
W2Dynamics interface. \fixme{For definitiness we compare the finite low
temperature results obtained via the two solvers: ED and CTQMC.  }
...
In panel (A) of \figu{figEx4} we report the evolution of the order
parameter 
...
In the following we show for a fixed value of $\lambda/D=1.0$ the adiabatic to anti-adiabatic crossover for the uniform solution of the Holstein problem on the Bethe lattice.

The Holstein interaction takes a simpler form for $\omega_0 \rightarrow \infty$ with $g^2/\omega_0=\lambda/2$ constant, also known as anti-adiabatic limit, for which it becomes a Hubbard attraction:
\begin{equation}
...
Meanwhile, in the opposite limit, $\omega_0 \rightarrow 0$ with $g^2/\omega_0=\lambda/2$ constant, also known as adiabatic limit, the system is in a phase of Bipolaronic insulator~\citep{Capone2006PRB} for $\lambda=1$.


\begin{figure}[ht!]
    \includegraphics[width=\linewidth]{edipack2_examples/Bethe_Holstein/figBethe_Holstein.pdf}
    \caption{\label{figEx5}Plots of the double occupancy (A) and the superconductive order parameter (B) as a function of $\omega_0/D$ for $\lambda/D=1$ in the normal (orange) and superconductive (red) phase. The horizontal broken line are the values for the corresponding Hubbard model in the anti-adiabatic limit.}
\end{figure}
