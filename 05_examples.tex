...
library and its interfaces as solver for DMFT through a variety of examples. We also discuss in detailed the relevant code snippets from different programming languages. 
Some tasks --particularly those associated to the implementation of the DMFT self-consistency--  are inherently independent of \NAME itself, and the impurity solver is agnostic about them. 
...
On the other hand, the results obtained with W2Dynamics rely on the internal framework provided by the software itself—see the \texttt{DMFT.py} script~\cite{Wallerberger2019CPC}—which handles all these tasks seamlessly.
...
\cite{3dXY_KMH}, as predicted by Hartree-Fock and $t$-$J$ perturbative expansions \cite{Rachel2010PRB} and there were two references to the same paper
