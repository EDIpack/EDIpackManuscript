...
$\rho(\e)=\frac{1}{2D}\sqrt{D^2-\e^2}$ This density of states does not normalize to 1,
...
allocation, Bethe lattice creation, and solver initialization. 'Bethe lattice creation' sounds a bit weird here. DMFT does not directly work with a lattice as a collection of sites, it only deals with the spectral functions.
...
\item[{\tiny {\it User}}] Update the Weiss field via the {\bf self-consistency}
  relation: $\GG^{-1}_0(i\omega) = G_\mathrm{loc}(i\omega) +
    \Sigma(i\omega)$ $G_\mathrm{loc}$ inverted?. For the Bethe lattice, this simplifies to
    $\Delta = \tfrac{D}{4}G_\mathrm{loc}$ Missing square in the coefficient?.
...
Capitalization in 'w2dynamics' is different compared to Sec. 4.
...
%
The main effect of the interaction to the topological properties can be captured by the renormalized mass term $M_\mathrm{eff}=M+\tfrac{1}{4}\Tr{\Re\Sigma(i\omega\to0)}$. The
...
\begin{lstlisting}[style=fstyle,numbers=none,basicstyle={\scriptsize\ttfamily}]
   ...
   do while(.not.converged.AND.iloop<nloop)
      ...
      !  - nonsu2 mode: broken Sz-conservation and we fit both spin components altogether
      select case(ed_mode)
       case("normal")
      ...
      !
      !> linear mixing and check convergence: using ${\color{comment-color}\mathrm{DMFTtools}}$
