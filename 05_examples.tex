...
library and its interfaces as a solver for DMFT through a variety of examples. We also discuss in detail the relevant code snippets from different programming languages. 
...
In the implementation using Fortran, Python or other \NAME API we adopt a reverse communication strategy, which requires the user to independently implement these parts or, equivalently, to exploit existing utilities. 
...
Similarly, the TRIQS library offers an extensive toolbox for the manipulation of Green’s functions and related quantities~\cite{Parcollet2015CPC}, making these tasks easily accessible while maintaining very high standards of optimization and accuracy.
...
\begin{tabular}{c|c}\label{list3}
{\bf Fortran} & {\bf C++}\\
  \stackinset{l}{}{t}{}{}{\listingA} & \stackinset{l}{}{t}{}{}{\listingB} 
\end{tabular}

The bath is described by the (unknown) function $\GG^{-1}_0$, called
...
$$ 
with  $\zeta=i\omega+\mu-h^0-\Sigma(i\omega)$.
...
do while(.not.converged.AND.iloop<nloop)
    ...
    !Check convergence: from $\mathrm{DMFTtools}$
...
\item Update the Weiss field's components, respectively 
...
In this example, we discuss  the normal and
...
\begin{equation} \label{eqex:H_Holstein}
    ...
    -\frac{1}{2}\right)\Big]. 
\end{equation}
We consider the half-filling regime of the particle-hole symmetric Bethe lattice density of states. We set the half-bandwidth as our energy unit $D=1$ and introduced the electron-phonon coupling $\lambda = \tfrac{2g^2}{\omega_0D}$.  
...
enters a Bipolaronic Insulating phase~\cite{Capone2006PRB} for our
...
the behavior for the normal solution ({\tt ed\_mode=normal}) against the superconductive
...
parameter $\phi$ in the adiabatic to anti-adiabatic crossover (panel (B) of \figu{figEx5}).
...
orbital electrons and holes across the band gap bind into a coherent phase.
...
The code implementation follows the same guidelines discussed above
for the other examples. There are two specific modifications required
from the user. The first is to generate the Hamiltonian $H(k)$ on a discretized Brillouin zone in
...
is constructed out of the matrix basis with 4 distinct elements
...
$E_x$ (we assume $E_y=0$ leveraging on residual $U(1)$ in-plane
symmetry). 
%
...
\begin{lstlisting}[style=fstyle,numbers=none,basicstyle={\scriptsize\ttfamily}]
   ...
   do while(.not.converged.AND.iloop<nloop)
     ...
     !> Get $G_\mathrm{loc}$ using $\color{comment-color}\mathrm{DMFTtools}$
     call get_gloc(Hk,Gmats,Smats,axis='m')
     !> Update the Weiss field ${\color{comment-color}\GG^{-1}=G^{-1}_\mathrm{loc}+\Sigma}$ +  Linear mixing: using $\color{comment-color}\mathrm{DMFTtools}$
...
%
The main effect of the interaction on the topological properties can be captured by the renormalized mass term $M_\mathrm{eff}=M+\tfrac{1}{4}\Tr{\Re\Sigma(i\omega\to0)}$. The
...
order this correction is proportional to $\tfrac{U-5J}{4}\langle
...
two orbitals get populated by one electron per site, reaching the
...
To complete the overview of \NAME's capabilities, we consider another
...
\begin{lstlisting}[style=fstyle,numbers=none,basicstyle={\scriptsize\ttfamily}]
   ...
   do while(.not.converged.AND.iloop<nloop)
      ...
      !
      !> Update local Weiss fields ${\color{comment-color}\GG^{-1}_0 = G_{loc} + \Sigma}$ :using ${\color{comment-color}\mathrm{DMFTtools}}$
      call dmft_self_consistency(Gmats,Smats,Weiss)
      !
      !> Fit the new bath:
...
is favored as the ground state of the system. In \figu{fig:KMenergy}(a)
