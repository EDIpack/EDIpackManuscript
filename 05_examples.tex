...
To demonstrate the capabilities of the W2Dynamics interface to \NAME,  here we briefly discuss how to solve the same problem, i.e. the Hubbard model on the Bethe lattice within DMFT W2Dynamics. 
In order to showcase the capabilities of the \NAME to address low-temperature problems comaring the W2Dynamics  Continuous-Time Quantum Monte Carlo (CTQMC) with hybridization expansion method against the \NAME solver at finite temperature.  

As we have seen the \NAME API, enabling access to the impurity solver and bath optimization procedures, requires the user to implement (using different methods) the DMFT algorithm.  W2Dynamics offers a structurally different solution, providing a single Python script {\tt DMFT.py} which takes care of the whole DMFT calculation via dedicated classes to implement generic self-consistency. The W2Dynamics is then entirely controlled by model-dependent parameters file {\tt Parameters.in} which contains a number of variable specifications, including non-default ones controlling the ED solver inherited from \NAME interface. Further information about the functioning of W2Dynamics can be found in Ref.\onlinecite{W2DynWallerberger}           
The two methods have another important difference. 
The first concerns the initial point of the iterative DMFT solution algorithm. 
While \NAME starts from a given discrete bath, W2Dynamics is initialized with a zero Self-energy function or, in alternative, read this quantity from a converged solution. When using \NAME ED interface, the initial Weiss is determined using self-consistency and finally an initial discrete bath is obtained through the \NAME bath optimization procedure. 
...
\end{lstlisting}
Following W2Dynamics  specifics, we run the {\tt DMFT.py} script as:
\begin{lstlisting}[style=mybash,numbers=noneccd ]
mpiexec -n 10 /opt/w2dynamics/DMFT.py bethe_dmft.in
python /opt/w2dynamics/hgrep latest siw-full -1
\end{lstlisting}
Where, as for W2Dyanamics, the results are collected into a HDF5 archive, including the output files inherited from \NAME which is inspected using the provided script {\tt hgrep}. In this example we extract the Matsubara self-energy function $\Sigma(i\omega_n)$ for the last DMFT iteration. 

\begin{figure}%[t!]
  \includegraphics[width=\linewidth]{figures/figBetheW2D.pdf}
    \caption{\label{figEx1W}%
    \textbf{Finite temperature DMFT solution.}
    Comparison of the imaginary parts of the Green's function $\Im{G}(i\omega_n)$ and Self-energy $\Im{\Sigma}(i\omega_n)$ from different solutions of the Hubbard model on the Bethe lattice using DMFT. Data are for $T/D=0.01$ and $U/D=2.0$. The results are obtained using original W2Dynamics CT-QMC solver and compared against the \NAME solver, either independently or using the W2Dynamics interface. 
        }
\end{figure}

In \figu{figEx1W} we report a comparison of the results obtained using the hybridization expansion CTQMC solver in W2Dynamics, the \NAME ED method available as solver W2Dynamics as well as independent calculation (Fortran API). Using DMFT, we solve the Hubbard model on a Bethe lattice for $T/D=0.01$ and $U=2.00$, corresponding to a correlated metallic state. 
In panel (A) we show the behavior of the imaginary part of the impurity Green's function $\Im{G}(i\omega_n)$ (note that W2Dynamics does not have access to real-axis Green's function). In panel (B) we show the same comparison for $\Im{\Sigma}(i\omega_n)$. 
The results obtained with the three methods are nearly identical already for a small number of bath levels ({\tt NBATH=7}) or low QMC statistics.   
...
Unlike the static mean-field description~\cite{Blason2021}, the
