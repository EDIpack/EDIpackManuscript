...
in complete analogy to the Python API, the global variables and functions are contained in a  {\tt struct} called {\tt global\_env}.
...
Functions are directly called from the library, making use of the {\tt  ccall} procedure. As previously stated, as a consequence of the C linking conventions multiple calls to {\tt  ccall} are necessary to account for the different input options of each interfaced Fortran procedure; the Julia wrapper functions take care of selecting the appropriate Fortran procedure depending on the set of input parameters provided by the user.
