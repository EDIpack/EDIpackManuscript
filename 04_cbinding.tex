...
{https://edipack.github.io/EDIpack2py}.
...
\begin{lstlisting}[style=mybash]
conda create -n edipack  #creation of a virtual environment (name can be changed)
conda activate edipack   #activation of the virtual environment
conda install -c conda-forge -c edipack edipack #installation from the edipack channel
\end{lstlisting}
...
{\tt bath\_type=normal,hybrid,general} (Sec.~\ref{sSecBath}). In addition, the
...
In order to use the \NAME impurity solver for a calculation instead of the built-in implementation of the CT-HYB algorithm, the user needs to set the {\tt solver} parameter in section {\tt General} to \NAME. 
