...
{\tt edipy2.global\_env.solve()} and therefore performs the bulk of
...
{\tt edipy2.global\_env.chi2\_fitgf()}. It accepts the function to fit (either
...
\NAME is supported as alternative impurity solver by the w2dynamics DMFT package\cite{Wallerberger2019CPC} available on \href{https://github.com/w2dynamics/w2dynamics}{GitHub}. The only prerequisite to use this option is the presence of a functioning EDIpy2 installation, which w2dynamics will attempt to import at runtime only if the user sets the appropriate configuration parameter.
